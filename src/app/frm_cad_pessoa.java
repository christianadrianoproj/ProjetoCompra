...
import banco.ServicoBancoPessoa;
 ...
 * @author Christian
 */
public class frm_cad_pessoa extends javax.swing.JFrame {
    ...
    private Pessoa pessoaSelecionada() {
        ...
        if (modoTela != MODO_INSERT) {
            idPessoa = Integer.parseInt(jTextFieldID.getText());
        }
        Date dt = new Date();
        ...
        if (jRadioPF.isSelected()) {
            p = new PessoaFisica(idPessoa, dt, jTextFieldNome.getText(), jTextCPF.getText(), Integer.parseInt(jTextRG.getText()));
        }

        if (jRadioPJ.isSelected()) {
            p = new PessoaJuridica(idPessoa, dt, jTextFieldNome.getText(), jTextCNPJ.getText(), Integer.parseInt(jTextIE.getText()));
        }
        return p;
    }
    ...
    private void mostraDadosSelecionando() {
        ...
        Pessoa p = srv.getPessoa(Integer.parseInt(String.valueOf(jTPessoas.getValueAt(jTPessoas.getSelectedRow(), 0))));
        if (srv.isPessoaJuridica(p)) {
            ...
            jTextFieldData.setText(df.format(pj.getDataNascimento()));
        }
        if (srv.isPessoaFisica(p)) {
            ...
            SimpleDateFormat df = new SimpleDateFormat("dd/MM/yyyy");
            jTextFieldData.setText(df.format(pf.getDataNascimento()));
        }
    }
    ...
    private void modoSalva() {
        ...
        ServicoPessoa srv = new ServicoPessoa();
        Pessoa p = pessoaSelecionada();
        srv.persistirBanco((Pessoa) p, modoTela == MODO_INSERT, modoTela == MODO_UPDATE, false);
        modoTela = -1;
        preparaBotoes();
        quantCad = carregaPessoas();
    }

    private int carregaPessoas() {
        int retorno = 0;
        ServicoBancoPessoa srvBancoPessoa = new ServicoBancoPessoa();
        ...
        try {
            retorno = srvBancoPessoa.getAll().size();
            for (Pessoa p : srvBancoPessoa.getAll()) {
                if (srvPessoa.isPessoaFisica(p)) {
                    PessoaFisica pes = (PessoaFisica) p;
                    modelo.addRow(new Object[]{pes.getIdPessoa(), pes.getCPF(), pes.getNome()});
                }
                if (srvPessoa.isPessoaJuridica(p)) {
                    PessoaJuridica pes = (PessoaJuridica) p;
                    modelo.addRow(new Object[]{pes.getIdPessoa(), pes.getCnpj(), pes.getNome()});
                }
            }
    ...
    private void initComponents() {
        ...
        jRadioPF.setText("Pessoa Física");
        ...
        jRadioPJ.setText("Pessoa Jurídica");
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(frm_cad_pessoa.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new frm_cad_pessoa().setVisible(true);
            }
        });
    }
